...
export class PlaceFitnessTrainerAppointmentComponent implements OnInit {
  ...
  fitnessForm = this.fb.group({
    firstname: [
      null,
      [Validators.required, Validators.pattern(this.wordSequence)],
    ],
    lastname: [
      null,
      [Validators.required, Validators.pattern(this.wordSequence)],
    ...
    pincode: [
      ...
      [
        ...
        Validators.pattern(this.numberSequence),
      ],
    ],
  ...
  amount(times) {
    switch (times) {
      case "one":
        this.fitnessForm.patchValue({ inr: 500, paisa: 0 });
        break;
      case "four":
        this.fitnessForm.patchValue({ inr: 1600, paisa: 0 });
        break;
      case "five":
        this.fitnessForm.patchValue({ inr: 1500, paisa: 0 });
        break;
      default:
        this.fitnessForm.patchValue({ inr: 0, paisa: 0 });
        break;
    }
  }
  ...
  onSubmit() {
      ...
      paisa,
      packages,
    } = this.fitnessForm.controls;
    
    if (this.fitnessForm.status === "VALID") {
      const fitnessObject = new Fitness(
        inr.value,
        paisa.value,
        streetaddress.value,
        city.value,
      ...
      if (this.navigatedToEdit) {
        ...
        this._userService
          .updatefitnessdata(fitnessObject, this.idToEdit)
          .subscribe((data) => {
           alert("Traniee updated");
          });
      } else {
        this._userService.postfitnessdata(fitnessObject).subscribe((data) => {
